import rospy
from realsense_segment2.srv import FindGraspingPoint, FindGraspingPointResponse, FindPlacingPoint, FindPlacingPointResponse
...
class ImageProcessor:
    def __init__(self):
        ...
        rospy.Service('find_placing_point', FindPlacingPoint, self.find_placing_point)
    ...
    def process_images(self, object_string, action_type):
...
# 0.539, 0.087, 0.634
            ...
            if action_type == 'pick':
                ...
                return FindPlacingPointResponse(pose_stamped)
            else:
                print("Error: Action not in [pick, place]")
    ...
    def find_grasping_point(self, req):
        ...
        while self.color_image_np is None and self.depth_image_np is None and self.point_cloud_data is None:
            rospy.sleep()        
        return self.process_images(object_string, 'pick')
    
    def find_placing_point(self, req):
        object_string = req.object_string
        rospy.loginfo("here: %s",object_string)
        while self.color_image_np is None and self.depth_image_np is None and self.point_cloud_data is None:
            rospy.sleep()        
        return self.process_images(object_string, 'place')
...
def find_placing_point_server():
    rospy.init_node('find_placing_point_server')
    image_processor = ImageProcessor()
    # rospy.Service('find_grasping_point', FindGraspingPoint, find_grasping_point)
    # rospy.loginfo("Ready to find grasping point.")
    rospy.spin()
