...
import rospy
from realsense_segment2.srv import FindGraspingPoint, FindPlacingPoint
...
def find_placing_point_client(object_string):
    rospy.wait_for_service('find_placing_point')
    find_grasping_point = rospy.ServiceProxy('find_placing_point', FindPlacingPoint)
    response = find_grasping_point(object_string)
    return response
...
supported_actions = ['pick', 'place', 'push'] # TODO: add push 


def grasp(object_string): 
...
def place(object_string): 
    result = find_placing_point_client(object_string)
    transformed_pose = transform_pose(result.pose_stamped, "base_link")
    transformed_pose.pose.orientation.x = 1
    transformed_pose.pose.orientation.y = 0
    transformed_pose.pose.orientation.z = 0
...
def main():
    rospy.init_node('natural_language_interaction_client')
    command = input("Please give the arm an instruction:\n")

    action, object_string = command.split(' ')
