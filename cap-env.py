...
class PushEnv():
  ...
  def step(self, action=None):
    ...
    # Set fixed primitive z-heights.
    hover_xyz = np.float32([obj_pos[0]-direction[0]*threshold, obj_pos[1]-direction[1]*threshold, 0.3])
    if obj_pos.shape[-1] == 2:
      push_xyz = np.append(obj_pos, 0.025)
    else:
      push_xyz = obj_pos
      push_xyz[2] = 0.025
    ...
    if destination_pos.shape[-1] == 2:
      destination_xyz = np.append(destination_pos, 0.15)
    else:
      destination_xyz = destination_pos
      destination_xyz[2] = 0.15
